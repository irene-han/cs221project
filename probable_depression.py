...
IN_FILE = "data_biomarkers_double_plus_one"
OUT_FILE = "data_biomarkers_double_plus_normalized"
...
age = [float(row[2]) for row in raw_data]
...
count = 0 
for i in range(0, len(raw_data)):
	if (raw_data[i][2] == "3") or (raw_data[i][2] == "4") or  (raw_data[i][2] == "5"):
		foo = data[i]
		foo.append("1")
		data[i] = foo
	elif (raw_data[i][2] == "0"):
